  ...
  <div class="mt-2">
    ...
    <div class="flex justify-between flex-wrap mt-3">
      <div v-for="item in recommendPlaylist" :key="item.id" class="break-words w-35 mb-4 relative " @click="handlePlayList(item)">
        <div class="playIcon bg-cover">
          <img :src="item.picUrl" class="w-35 h-35 rounded-1 cursor-pointer bg-cover">
          <div i-carbon-play-outline-filled color="#ec4141" w-8 h-8 bottom-18 right-2 absolute class="playIconChild" />
        </div>

        <span class="mt-2 inline-block color-[#333333] hover:color-[black] hover:font-500 cursor-pointer line-clamp-2">
